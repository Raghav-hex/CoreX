# scheduler/main.py (Complete for Preemption History feature)
...
async def schedule_jobs_periodically():
    ...
    while True:
        try:
            ...
            logger.info(f"Popped job '{job_id}'. Looking for a suitable agent.")
            job = db.query(database.Job).filter(database.Job.job_id == job_id).first()
            if not job:
                logger.error(f"Job '{job_id}' not in DB. Skipping.")
                continue

            preempted_list = job.preempted_by_agents
            idle_agents_ids = [
                agent_id for agent_id, agent_info in connected_agents.items()
                if agent_info["status"] == "idle" and agent_id not in preempted_list
            ]

            if not idle_agents_ids:
                logger.warning(f"No SUITABLE agents for '{job_id}'. Re-queuing.")
                await redis_client.lpush("job_queue", job_id)
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        while True:
            ...
            elif msg_type == "task_preempted":
                task_id = message.get("task_id")
                logger.warning(f"Received preemption for task '{task_id}' from agent '{agent_id}'.")
                job = db.query(database.Job).filter(database.Job.job_id == task_id).first()
                if job:
                    job.status = "queued"
                    job.assigned_agent_id = None
                    job.preempted_by_agents.append(agent_id)
                    flag_modified(job, "preempted_by_agents")
                    db.commit()
                    await redis_client.lpush("job_queue", job.job_id)
                    if agent_id in connected_agents:
                        connected_agents[agent_id]["status"] = "idle"
                    logger.info(f"Re-queued '{task_id}'. Agent '{agent_id}' is now blacklisted for this job.")
